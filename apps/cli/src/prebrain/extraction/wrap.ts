...
// same reason: untrusted captured text gets sanitized (sanitize.ts, the
// deleted pipeline's Global Rule 2b) AND wrapped in an explicitly
// labeled block (Global Rule 2a) so the model is told twice -- once
// structurally, once in prose -- that this is data to extract from, not
// instructions to follow. Both layers matter: sanitize.ts narrows the
...
// this function -- fix-plan-v3 2.3's "all source text is wrapped in the
// delimited data-block convention on every model call, both modes" is
