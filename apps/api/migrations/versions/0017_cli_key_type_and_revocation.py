...
name to "cli", nothing enforces or reads it back). Security fix-plan item
C1 needs a CLI-specific list/revoke surface (CLI keys currently can't be
revoked server-side at all -- `gnt logout` only ever deleted the local
credentials file), so this adds an explicit key_type column and backfills
