...
server-side credential a scheduled job reads on its own timetable, the
same shape as Slack's bot token or GitHub's PAT — not something a human
runs a one-shot `gnt prebrain` walker against from their own device the
way the MCP-in connectors under apps/cli/src/prebrain do.

Reads three kinds of prose from a customer's own Zendesk instance: macro
action text, help-center article bodies, and internal (non-public) notes
on recently-updated tickets. Never reads a ticket RECORD itself
(requester, submitter, assignee, organization, group, custom fields,
tags, subject, priority, status, satisfaction rating, ...) — see
gnt.zendesk.client's TicketRef dataclass and its own docstring for how
that's enforced structurally, and tests/test_zendesk_client.py's
declared-fields test for the proof.

Follows workers/tasks_staleness.py's reference two-tier session pattern
for every cron job in this codebase: enumerate org ids through
get_cron_sessionmaker() (BYPASSRLS, the narrow gnt_cron exemption), then
do every actual per-org read/write through a normal scope_to_org'd
session, one org at a time, never a single cross-org statement.

Pipeline per org, per content item:
  1. Skip if this exact (item, content) pair has already been through
     extraction before (gnt.zendesk_sync_status.has_been_processed) — the
     dedup gate that keeps a large, mostly-unchanged Zendesk instance from
     re-extracting its whole macro library and ticket history every night.
  2. Run the raw text through the server-side privacy gate
     ...
     task requires masking to happen before extraction, not merely before
     storage, since a support macro or internal note is exactly the
  ...
  4. Extract zero or more candidate rules from the masked text
     (gnt.pipeline.content_extraction).
  5. For each candidate: create a draft rule (routers/rules.py's
     ...
     workers/tasks_contradictions.py and workers/tasks_staleness.py
     already give their own sweep-generated drafts. An org with Zendesk
     connected but no GitHub repo yet still gets its candidate drafts
     created and submitted — visible in the normal review queue — it just
     has nowhere to open a PR against until GitHub is connected too,
     exactly like a human-submitted draft rule with no repo connected.
  6. Record the item as processed regardless of whether extraction
     produced any candidates, so a macro with genuinely no policy content
     in it isn't re-sent to the model every single night.

Bounded per org per run by Settings.zendesk_sweep_max_items_per_org —
macros, changed tickets' internal notes, and help-center articles
combined — same founder-tunable-knob convention
contradiction_sweep_max_comparisons_per_org already establishes.

Errors skip-and-report, never crash the worker: one bad item (a
malformed Zendesk payload, an unreachable ticket, an LLM failure) skips
that one item and keeps going (_process_item's own try/except); a
credential going bad mid-run (Zendesk revokes a token, an outage) fails
that one org's sync and is recorded via zendesk_sync_states' last_error,
never raised past sync_zendesk_for_org; one org's failure never stops the
rest of the night's orgs (sync_zendesk's own per-org loop)."""
...
from gnt.db.models import GithubConnection, Org, ZendeskConnection
...
from gnt.zendesk.client import ZendeskClientError, list_articles, list_internal_notes, list_macros, list_recently_updated_ticket_ids
from gnt.zendesk.crypto import decrypt_token as decrypt_zendesk_token
from gnt.zendesk_sync_status import has_been_processed, record_processed, record_sync_result

# Same cross-worker "no real human behind this write" actor id convention
# as tasks_contradictions.py's _SWEEP_ACTOR_ID / tasks_staleness.py's
# _SWEEP_ACTOR_ID.
_SYNC_ACTOR_ID = "gnt:zendesk-sync"

# How far back the incremental ticket export looks for "recently updated"
# tickets whose internal notes are worth checking — a fixed rolling
# window, not "since the last successful run", so a sync that's been
# broken for a while and then recovers doesn't try to walk an unbounded
# backlog once it does.
...
async def sync_zendesk(ctx: dict[str, Any]) -> None:
    """arq cron entrypoint (see worker.py's cron_jobs). Enumerates every
    org, then runs that org's own Zendesk sync — see module docstring for
    why enumeration and the per-org work use two different sessions/
    roles."""
    ...
    for org_id in org_ids:
        await sync_zendesk_for_org(org_id)


async def sync_zendesk_for_org(org_id: str) -> None:
    """The per-org unit of work sync_zendesk loops over. An org with no
    connected Zendesk instance is skipped outright, same "not an error"
    ...
    zendesk_sync_states row for this run, success or failure — see this
    module's own docstring and gnt.zendesk_sync_status.record_sync_result."""
    session_factory = get_sessionmaker()
    async with session_factory() as session:
        await scope_to_org(session, org_id)
        connection = (
            await session.execute(select(ZendeskConnection).where(ZendeskConnection.org_id == org_id))
        ).scalar_one_or_none()
        ...
        try:
            ...
            # Org-level failure (bad/revoked credentials, Zendesk outage) —
...
async def _iter_content_items(subdomain: str, agent_email: str, api_token: str, budget: int):
    """Lazily yields (item_type, item_id, text, source_label, source)
    tuples across macros, help-center articles, and recently-updated
    tickets' internal notes, in that order, stopping as soon as `budget`
    items total have been yielded — so a budget-exhausted run never even
    makes the Zendesk API calls for whichever source comes later in the
    order. item_type is one of "macro" | "article" | "internal_note",
    matching zendesk_processed_items.item_type."""
    yielded = 0

    for macro in await list_macros(subdomain, agent_email, api_token):
        if yielded >= budget:
            return
        yield (
            "macro", macro.id, macro.action_text,
            f"Zendesk macro: {macro.title}", f'Zendesk macro "{macro.title}"',
        )
        yielded += 1

    if yielded >= budget:
        return
    for article in await list_articles(subdomain, agent_email, api_token, limit=budget - yielded):
        if yielded >= budget:
            return
        yield (
            "article", article.id, article.body_text,
            f"Zendesk help center article: {article.title}",
            article.html_url or f'Zendesk article "{article.title}"',
        )
        yielded += 1

    if yielded >= budget:
        return
    start_time_unix = int((datetime.now(timezone.utc) - timedelta(days=_TICKET_LOOKBACK_DAYS)).timestamp())
    ticket_refs = await list_recently_updated_ticket_ids(
        subdomain, agent_email, api_token, start_time_unix=start_time_unix, limit=budget - yielded
    )
    for ticket in ticket_refs:
        if yielded >= budget:
            return
        try:
            notes = await list_internal_notes(subdomain, agent_email, api_token, ticket_id=ticket.id)
        except ZendeskClientError as exc:
            # One ticket's comments failing to fetch (deleted mid-sync,
            # transient error) skips just that ticket — never the rest of
            # the org's sync.
            sentry_sdk.capture_exception(exc)
            continue
        for note in notes:
            if yielded >= budget:
                return
            yield (
                "internal_note", f"{ticket.id}:{note.comment_id}", note.body_text,
                f"Zendesk internal note on ticket #{ticket.id}",
                f"Zendesk ticket #{ticket.id} (internal note)",
            )
            yielded += 1


async def _run_sync(session: AsyncSession, org_id: str, connection: ZendeskConnection) -> tuple[int, int]:
    settings = get_settings()
    api_token = decrypt_zendesk_token(connection.api_token_encrypted)
    ...
    # propagates up to sync_zendesk_for_org's own broad except -- same
    # org-level failure treatment as a bad Zendesk credential or a Zendesk
    # outage, not something this function needs to catch itself.
...
async def _process_item(
    ...
    *,
    ...
    applies to its own per-item loop: a malformed Zendesk payload, an LLM
...
async def _create_and_propose_candidate(
    ...
    rule = await create_draft_rule(
        org_id,
        _SYNC_ACTOR_ID,
        CreateRuleRequest(title=candidate.title, body=candidate.body, source=source, tags=["zendesk"]),
    ...
    if github_connection is not None:
        try:
            ...
            # Best-effort — same discipline tasks_staleness.py's
            # _propose_flag documents: the draft already exists and is
            # already submitted (visible in the normal review queue)
            # regardless of whether opening a PR for it succeeds.
            sentry_sdk.capture_exception(exc)
