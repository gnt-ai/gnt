...
from gnt.db.models import GithubConnection, IntercomConnection, IntercomSyncState, OnboardingEvent, Org
...
from gnt.intercom.client import Article, IntercomClientError, SavedReply
from gnt.intercom.crypto import encrypt_token as encrypt_intercom_token
from gnt.intercom_sync_status import has_been_processed
from gnt.pipeline.content_extraction import RuleCandidate
from gnt.store_client import list_rules as store_list_rules
from gnt.workers import tasks_intercom
from gnt.workers.tasks_intercom import sync_intercom, sync_intercom_for_org
...
@pytest.fixture
def org_a() -> str:
    return f"org_test_intercom_{uuid.uuid4().hex[:8]}"
...
async def _test_db_sessionmaker(monkeypatch):
    ...
    monkeypatch.setattr(tasks_intercom, "get_sessionmaker", lambda: session_factory)
...
async def _seed_intercom_connection(org_id: str) -> None:
    engine = create_async_engine(TEST_DATABASE_URL)
    try:
        session_factory = async_sessionmaker(engine, expire_on_commit=False)
        async with session_factory() as session:
            ...
            session.add(
                IntercomConnection(
                    org_id=org_id,
                    access_token_encrypted=encrypt_intercom_token("fake-token"),
...
async def _cleanup_org(org_id: str) -> None:
    ...
    try:
        ...
        async with session_factory() as session:
            ...
            from gnt.db.models import IntercomProcessedItem

            await session.execute(
                IntercomProcessedItem.__table__.delete().where(IntercomProcessedItem.org_id == org_id)
            )
            await session.execute(IntercomSyncState.__table__.delete().where(IntercomSyncState.org_id == org_id))
            await session.execute(IntercomConnection.__table__.delete().where(IntercomConnection.org_id == org_id))
...
@pytest.fixture
def _no_articles_or_conversations(monkeypatch):
    """Most tests below only care about saved-reply content — stub the
    other two sources to empty so a test asserting "exactly one item
    processed" isn't accidentally sensitive to whatever list_articles/
    list_recently_updated_conversation_ids would otherwise be mocked to."""

    async def _empty_articles(access_token, *, limit):
        return []

    async def _empty_conversations(access_token, *, start_time_unix, limit):
        return []

    monkeypatch.setattr(tasks_intercom, "list_articles", _empty_articles)
    monkeypatch.setattr(tasks_intercom, "list_recently_updated_conversation_ids", _empty_conversations)


@pytest.fixture
def _one_saved_reply(monkeypatch, _no_articles_or_conversations):
    async def _replies(access_token):
        return [
            SavedReply(id="55", title="Refund policy", action_text="Refunds within 30 days.", updated_at="x")
        ]

    monkeypatch.setattr(tasks_intercom, "list_saved_replies", _replies)
...
def _mock_extraction(monkeypatch):
    ...
    monkeypatch.setattr(tasks_intercom, "extract_candidate_rules_async", _extract)
    return state, calls


@pytest.fixture
def _mock_propose_pr(monkeypatch):
    """Same call-boundary mocking shape test_tasks_zendesk.py's own
...
async def test_org_with_no_intercom_connection_is_skipped(_test_db_sessionmaker, org_a):
    engine = create_async_engine(TEST_DATABASE_URL)
    try:
        session_factory = async_sessionmaker(engine, expire_on_commit=False)
        async with session_factory() as session:
    ...
    try:
        await sync_intercom_for_org(org_a)
        async with _test_db_sessionmaker() as session:
            await scope_to_org(session, org_a)
            row = (
                await session.execute(select(IntercomSyncState).where(IntercomSyncState.org_id == org_a))
            ).scalar_one_or_none()
...
async def test_candidate_created_as_draft_and_submitted_without_github(
    _test_db_sessionmaker, org_a, _one_saved_reply, _mock_extraction
):
    ...
    await _seed_intercom_connection(org_a)
    ...
    try:
        ...
        assert drafts[0]["tags"] == ["intercom"]
        assert "Intercom saved reply" in drafts[0]["source"]
...
async def test_candidate_proposed_as_a_real_pr_when_github_is_connected(
    _test_db_sessionmaker, org_a, _one_saved_reply, _mock_extraction, _mock_propose_pr
):
    await _seed_intercom_connection(org_a)
    ...
    try:
        ...
        async with _test_db_sessionmaker() as session:
            processed = await has_been_processed(
                session, org_a, "saved_reply", "55",
                tasks_intercom._fingerprint("saved_reply", "55", "Refunds within 30 days."),
            )
...
async def test_one_bad_item_does_not_block_the_rest_of_the_sync(
    _test_db_sessionmaker, org_a, _no_articles_or_conversations, monkeypatch
):
    async def _replies(access_token):
        return [
            SavedReply(id="1", title="Broken", action_text="broken content", updated_at="x"),
            SavedReply(id="2", title="Healthy", action_text="healthy content", updated_at="x"),
        ]
...
async def test_org_level_intercom_failure_is_recorded_as_a_sync_error(
    _test_db_sessionmaker, org_a, monkeypatch
):
    async def _boom(access_token):
        raise IntercomClientError("Intercom returned 401: token revoked")

    monkeypatch.setattr(tasks_intercom, "list_saved_replies", _boom)
    await _seed_intercom_connection(org_a)

    try:
        await sync_intercom_for_org(org_a)  # must not raise
        async with _test_db_sessionmaker() as session:
            await scope_to_org(session, org_a)
            state = (
                await session.execute(select(IntercomSyncState).where(IntercomSyncState.org_id == org_a))
...
async def test_budget_bounds_the_number_of_items_processed(
    ...
    monkeypatch.setattr(get_settings(), "intercom_sweep_max_items_per_org", 2)

    async def _replies(access_token):
        return [
            SavedReply(id=str(i), title=f"Reply {i}", action_text=f"content {i}", updated_at="x") for i in range(5)
        ]
...
async def test_llm_quota_exhausted_stops_the_run_early(
    ...
    async def _replies(access_token):
        return [SavedReply(id=str(i), title=f"Reply {i}", action_text=f"content {i}", updated_at="x") for i in range(3)]

    async def _exhausted(org_id, **kwargs):
        return False

    monkeypatch.setattr(tasks_intercom, "list_saved_replies", _replies)
    monkeypatch.setattr(tasks_intercom, "check_llm_quota", _exhausted)
    _state, calls = _mock_extraction
    await _seed_intercom_connection(org_a)
...
async def test_articles_are_read_with_html_stripped_and_never_exceed_budget(
    ...
    async def _no_replies(access_token):
        return []

    async def _articles(access_token, *, limit):
        return [
            Article(
                id="7", title="Shipping", body_text="Orders ship within 2 days.",
                html_url="https://acme.intercom.help/articles/7", updated_at="x",
            )
        ]

    async def _no_conversations(access_token, *, start_time_unix, limit):
        return []

    monkeypatch.setattr(tasks_intercom, "list_saved_replies", _no_replies)
    monkeypatch.setattr(tasks_intercom, "list_articles", _articles)
    monkeypatch.setattr(tasks_intercom, "list_recently_updated_conversation_ids", _no_conversations)
    await _seed_intercom_connection(org_a)
    ...
    try:
        ...
        drafts = await store_list_rules(org_a, status="in_review")
        assert len(drafts) == 1
        assert drafts[0]["source"] == "https://acme.intercom.help/articles/7"
    finally:
        await _cleanup_org(org_a)


async def test_sync_intercom_processes_each_org_in_its_own_scope(
    _test_db_sessionmaker, org_a, org_b, _no_articles_or_conversations, _mock_extraction, monkeypatch
):
    get_cron_engine.cache_clear()
    get_cron_sessionmaker.cache_clear()

    # Both orgs share the same list_saved_replies mock -- distinguished by
    # access token, which each org's own IntercomConnection carries.
    calls_by_token: dict[str, str] = {}

    async def _replies(access_token):
        label = calls_by_token.get(access_token, "?")
        return [SavedReply(id="1", title=f"Org {label} reply", action_text=f"content for {label}", updated_at="x")]
    ...
    for org_id, token, label in ((org_a, "tok-a", "A"), (org_b, "tok-b", "B")):
        calls_by_token[token] = label
        engine = create_async_engine(TEST_DATABASE_URL)
        try:
            session_factory = async_sessionmaker(engine, expire_on_commit=False)
            async with session_factory() as session:
                ...
                session.add(
                    IntercomConnection(
                        org_id=org_id,
                        access_token_encrypted=encrypt_intercom_token(token),
                        installed_by_user_id="admin",
                    )
                )
                await session.commit()
        finally:
