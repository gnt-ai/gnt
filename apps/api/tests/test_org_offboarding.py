...
async def _seed_full_org(db_session, org_id: str) -> None:
    ...
    db_session.add(
        NotionConnection(
            org_id=org_id,
            access_token_encrypted=encrypt_notion_token("fake-notion-token"),
            installed_by_user_id="seed_user",
        )
    )
    db_session.add(
        LinearConnection(
            org_id=org_id,
            access_token_encrypted=encrypt_linear_token("fake-linear-token"),
            installed_by_user_id="seed_user",
        )
    )
    db_session.add(
        IntercomConnection(
            org_id=org_id,
            access_token_encrypted=encrypt_intercom_token("fake-intercom-token"),
            installed_by_user_id="seed_user",
        )
    )
    db_session.add(IntercomSyncState(org_id=org_id))
    db_session.add(
        IntercomProcessedItem(
            org_id=org_id, item_type="saved_reply", item_id="1", content_fingerprint="fp1"
        )
    )
    db_session.add(
        ZendeskConnection(
            ...
            api_token_encrypted=encrypt_zendesk_token("fake-zendesk-token"),
            installed_by_user_id="seed_user",
        )
    )
    db_session.add(ZendeskSyncState(org_id=org_id))
    db_session.add(
        ZendeskProcessedItem(org_id=org_id, item_type="macro", item_id="1", content_fingerprint="fp1")
    )
    db_session.add(LlmUsage(org_id=org_id, month=date.today().replace(day=1)))
    db_session.add(
        StalenessRefreshProposal(
            org_id=org_id,
            rule_slug="rules/legacy-1",
            ...
            new_rule_slug="rules/legacy-2",
            pr_number=1,
            pr_url=f"https://github.com/acme/{org_id}/pull/1",
        )
    )
