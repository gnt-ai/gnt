...
Two honest ways through this today, neither of them polished:

1. **Run `apps/web` separately** (its own `pnpm dev` / build, pointed at
   this API via its own env) alongside the compose stack, just for the
   login page. Not part of `docker-compose.yml` and not tested as part of
   this change.
2. **Mint a key by hand**, the same way `apps/api/DEPLOY.md`'s own P0
   acceptance test bootstraps a throwaway admin org (see that file's
   "Getting `E2E_ADMIN_KEY`" section) — insert an `orgs` row and an
   `mcp_api_keys` row directly via `psql` against the compose Postgres, with
   a `gnt_live_`-prefixed key whose SHA-256 hash you compute locally, then
   write it to `~/.gnt/credentials.json` yourself (`{"api_key": "...",
   "key_id": "..."}`, the same shape `saveApiKey()` in
   `apps/cli/src/credentials.ts` writes). Manual, but it's the same
   mechanism DEPLOY.md already documents for exactly this "no live session
   to mint a key from" situation — nothing new invented here.
...
Groq/ZeroEntropy/GitHub keys, with `apps/api/DEPLOY.md`'s documented
Postgres role split (`gnt_app`/`gnt_cron`/`gnt_admin`, row-level security)
as the hardening path before production traffic. You own uptime, upgrades,
and support for your own deployment.

The hosted product (gnt.ai) is two flat-rate tiers — confirmed against
`apps/web/app/pricing/page.tsx` and `apps/api/src/gnt/plan_limits.py`, not
invented: **Base** ($29/mo, 1,500 `check_action` calls/month, 14-day free
trial) and **Pro** ($149/mo, 8,000 calls/month, no trial, the only tier
that allows multi-org membership). Beyond the usage cap difference, the
marketing site doesn't currently spell out specifics like support SLAs or
uptime guarantees — the one line of copy that exists on this
(`apps/web/app/page.tsx`'s homepage FAQ) says self-hosting is "not today"
possible ("gnt.ai runs as a hosted service, so you never touch
infrastructure"). That's stale relative to this page and the root
README's own self-host quickstart, which this change doesn't touch (out of
scope) — flagging the inconsistency here rather than papering over it.
Worth reconciling on the marketing side; until then, this page and the
root README are the accurate statement of what self-hosting actually
supports.
