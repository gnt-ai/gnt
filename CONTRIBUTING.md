...
That covers `apps/web` and `apps/cli` (the pnpm workspace). Two apps need their own setup on
top of that:
...
`.github/workflows/ci.yml` runs lint/typecheck/build/test for `apps/web`, `apps/cli`, and
`apps/store`, plus ruff + pytest for `apps/api`, on every push and PR. `.github/workflows/security.yml`
